...
public class DrawInfo extends Draw {
	...
	static {
		color1.setColor(0xFFAFB7AB);
		color2.setColor(0xFF6D7581);
		color3.setColor(0xFF434A64);
		color4.setColor(0xFF242A45);
		color5.setColor(0xFF12142F);
	}
	...
	public  int   a      = (int) mainBase.activity.getDensity();
	public  int   h      = mA + 8 * a;
	public  int   w      = w();
	private int   mW     = w - a * 7 - mA;
	private int color;
	...
	private void drawLeftPart(Canvas canvas, int h, int w) {
		// float nh = h / a;
		// float nw = w / a;
		canvas.drawRect(0, 0, w, h, color3);
		canvas.drawRect(a, a, w - a, h - a, color1);
		canvas.drawRect(3 * a, 3 * a, w - 3 * a, h - 3 * a, color4);
	...
	private void drawRightPart(Canvas canvas, float h, float w, float mW) {
		canvas.drawRect(mW, 0, w, h, color3);
		canvas.drawRect(mW + a, a, w - a, h - a, color1);
		float a3 = a * 3;
		canvas.drawRect(mW + a3, a3, w - a3, h - a3, color3);
	}
	...
	public void draw(Canvas canvas) {
		...
		if (true) {
			...
			int bitmapY = a * 4;
			int bitmapX = mW + a * 3;
			canvas.scale(mScale, mScale, bitmapX, bitmapY);
			...
			int y = bitmapY + A - defenceNumberBitmap.getHeight();
			int x = bitmapX + A - defenceNumberBitmap.getWidth();
		...
		for (int i = 0; i < number; i++) {
			paint.setColor(color & 0x00FFFFFF | 0xFF * (number - i) / number << 24);
		...
		float yGold = (h - Images().amountGoldH * mScale) / 2;
		float yUnits = (h - Images().amountUnitsH * mScale) / 2;

		canvas.save();
		canvas.scale(mScale, mScale, xGold, yGold);
		canvas.drawBitmap(Images().amountGold, xGold, yGold, null);
		canvas.drawBitmap(goldBitmap, xGold + Images().amountGoldW + a, yGold, null);
		canvas.restore();

		canvas.save();
		canvas.scale(mScale, mScale, xUnits, yUnits);
