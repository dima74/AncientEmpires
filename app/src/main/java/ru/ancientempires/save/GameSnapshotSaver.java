...
public class GameSnapshotSaver
{
	...
	private void saveNamedPoints() throws Exception
	{
		JsonWriter writer = loader.getWriter("namedPoints.json");
		writer.beginObject();
		for (Entry<String, AbstractPoint> entry : game.namedPoints.objects.entrySet())
		{
			writer.name(entry.getKey());
			ReflectionSaver.save(writer, entry.getValue());
		}
		writer.endObject();
		writer.close();
	}

	private void saveNamedBooleans() throws Exception
	{
		JsonWriter writer = loader.getWriter("namedBooleans.json");
		writer.beginObject();
		for (Entry<String, Boolean> entry : game.namedBooleans.objects.entrySet())
