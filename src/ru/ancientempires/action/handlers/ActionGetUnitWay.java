...
public class ActionGetUnitWay extends ActionFrom
{
	...
	public void createWay(final int startI, final int startJ)
	{
		...
		while (true)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				final int nextI = minI + ActionGetUnitWay.addI[k];
				final int nextJ = minJ + ActionGetUnitWay.addJ[k];
				...
				if (nextUnit != null && nextUnit.player != unit.player)
					continue;
				final int cellWeight = ActionGetUnitWay.getDifficulte(unit, nextICoord, nextJCoord);
		...
		result.setProperty("visibleField", is);
		result.setProperty("realField", realIs);
		result.setProperty("prevI", prevI);
		result.setProperty("prevJ", prevJ);
	}
