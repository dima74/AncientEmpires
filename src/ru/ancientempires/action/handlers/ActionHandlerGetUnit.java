...
public class ActionHandlerGetUnit extends ActionHandler
{
	
	public ActionHandlerGetUnit(ActionType type)
	{
		super(type);
	}
	...
	{
		return new ActionHandlerGetUnit(type);
	}
	...
	public void createWay()
	{
		...
		while (true)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				final int nextI = minDistanceI + ActionHandlerGetUnit.addI[k];
				final int nextJ = minDistanceJ + ActionHandlerGetUnit.addJ[k];
				...
				if (nextUnit != null && nextUnit.player.team != unit.player.team)
					continue;
				final int steps = ActionHandlerGetUnit.getSteps(unit, nextIAbsolute, nextJAbsolute);
